.PHONY: help dev-up dev-down migrate-up migrate-down dev-api dev-admin dev-scraper dev-settler gen-api gen-admin gen-web-client gen-admin-client build-all dev-docs
...
dev-admin: ## Run Admin service (hot reload)
	cd backend && PORT=8081 go run ./services/admin/cmd/main.go
...
gen-api: ## Generate Go types from API spec
	oapi-codegen -package handler -generate types docs/api-reference/user-api.yaml > backend/services/api/internal/handler/types_gen.go

gen-admin: ## Generate Go types from Admin spec
	oapi-codegen -package handler -generate types docs/api-reference/admin-api.yaml > backend/services/admin/internal/handler/types_gen.go

gen-web-client: ## Generate TypeScript client for web frontend
	cd frontend/web && npx openapi-typescript ../../docs/api-reference/user-api.yaml -o src/lib/api/schema.d.ts

gen-admin-client: ## Generate TypeScript client for admin frontend
	cd frontend/admin-web && npx openapi-typescript ../../docs/api-reference/admin-api.yaml -o src/lib/api/schema.d.ts
...
# Docs
dev-docs: ## Start Mintlify dev server
